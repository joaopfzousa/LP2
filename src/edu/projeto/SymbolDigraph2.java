...
public class SymbolDigraph2 {
     ...
     * @param empresasST St airport
     */
    public SymbolDigraph2(String filename, String delimiter, Integer id_empresa, SeparateChainingHashST<Integer, Empresa> empresasST) {
        ...
        while (in.hasNextLine()) {
            ...
            if (empresasST.get(id_empresa).getIdEmpresa() == id_empresa) {
                for (int i = 1; i < a.length; i = i + cost + 1) {
                    int w = st.get(a[i]);
                    if (empresasST.get(id_empresa).getIdEmpresa() == id_empresa) {
                        graph.addEdge(v, w);
                    }
